...
/**
 * 递归清理 undefined，避免 Next.js getStaticProps 序列化报错
 * - 对象字段为 undefined 时直接移除
 * - 数组项为 undefined 时转为 null（与 JSON 行为一致）
 */
function sanitizeUndefinedForNext(value) {
  if (value === undefined) {
    return undefined
  }
  if (value === null) {
  ...
  if (Array.isArray(value)) {
    return value.map(item => {
      ...
      const sanitizedItem = sanitizeUndefinedForNext(item)
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  return sanitizeUndefinedForNext(props)
}
...
function getSiteInfo({ collection, block, rawMetadata, NOTION_CONFIG }) {
  ...
  const defaultDescription = NOTION_CONFIG?.DESCRIPTION || 'Signal > Noise'
